...
In this phase, there is a while in [compiler.py](https://github.com/ArshiAAkhavan/C-minus-compiler/blob/master/compiler.py) that calls a function in [scanner.py](https://github.com/ArshiAAkhavan/C-minus-compiler/blob/master/scanner/scanner.py#L12) (get_next_token()) to read the next character from input file; in future phases this function will be called by parser. get_next_token() function does this by the help of [buffer_reader.py](https://github.com/ArshiAAkhavan/C-minus-compiler/blob/master/scanner/buffer_reader.py); buffer_reader inputs a file and returns its characters one by one. The line number of the input file is also tracked ,for reporting errors and tokens, by counting the number of "\n"'s in the file.
...
## Test Scanner
The file [test_scanner.py](https://github.com/ArshiAAkhavan/C-minus-compiler/blob/master/test_scanner.py) uses the samples folder to see if the expected outputs are generated from each input and reports the results in log.
## How to Use
